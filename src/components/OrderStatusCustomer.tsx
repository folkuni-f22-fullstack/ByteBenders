...
export default function OrderStatusCustomer({ props }) {
    ...
    function getOrderNumber() {
        let order = JSON.parse(getKeyFromLS("order"));
        console.log("orderNumber: ", order[0].orderId);
        !order && (order = localStorage.setItem("orderNumber-pending", "1"));

        const orderNumber = order[0].orderId;
        return orderNumber;
    }
        ...
        <div className="CustomerStatusOrder">
            <div className={props.waiting ? "order-waiting" : "order-finished"}>
                ...
                {props.waiting ? (
                    <h3>
                        {" "}
                        ETA: <span className="yellow"> {count} </span> minutes
                    </h3>
                ) : (
